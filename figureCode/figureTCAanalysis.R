...
args <- commandArgs(TRUE) # args <- c("../figureData/female.wgcna.RData", "../figureData/gene_go.table", "../figureData/gene.info")
...
file.width = 89
cairo_pdf(file = args[4], width = file.width/25.4, height = file.width/25.4, family = args[5])
par(las = 1, tcl = -0.2, mar = c(0.5, 0.5, 0.5, 0.5), ps = 7, lwd = 0.5)
layout(mat = matrix(c(1, 2, 3, 4, 5, 6), ncol = 3, byrow = TRUE), widths = c(1, 0.3, 1))
...
young.cor <- blup.25c.cor[bp.go.gene, bp.go.gene]
old.cor <- blup.18c.cor[bp.go.gene, bp.go.gene]
...
young.order.cor <- orderCor(young.cor, blup.25c.tree[bp.go.gene])
young.order.module <- young.order.cor$mod
old.order.cor <- old.cor[rownames(young.order.cor$cor), colnames(young.order.cor$cor)]
names(blup.18c.tree) <- names(blup.25c.tree)
old.order.module <- blup.18c.tree[rownames(young.order.cor$cor)]

par(las = 1, tcl = -0.2, mar = c(1, 1, 1, 1), ps = 7, lwd = 0.5)

plot(c(0, length(bp.go.gene)), c(0, length(bp.go.gene)), type = "n", axes = FALSE, xlab = "", ylab = "", asp = 1)
image(1:length(bp.go.gene), 1:length(bp.go.gene), young.order.cor$cor, col = colorRampPalette(c(brewer.pal(9, "Blues")[9], "white", brewer.pal(9, "Reds")[9]))(101), useRaster = F, add = TRUE)
...
text(1:length(bp.go.gene) + 1.1, rep(0, length(bp.go.gene)), gene.info[rownames(young.order.cor$cor), 2], font = 3, srt = 45, pos = 2, xpd = TRUE, col = ifelse(gene.info[rownames(young.order.cor$cor), 2] %in% c("kdn", "ScsbetaA"), "red", "black"))
...
text(0, length(bp.go.gene) + 2, "Young", pos = 4, cex = 7/par("ps")/par("cex"), xpd = TRUE)
...
image((1:length(cor20))/20, c(0.72, 0.74), matrix(rep(cor20, 2), ncol = 2, byrow = F), col = colorRampPalette(c(brewer.pal(9, "Blues")[9], "white", brewer.pal(9, "Reds")[9]))(20), add = TRUE, useRaster = F)
...
par(las = 1, tcl = -0.2, mar = c(1, 1, 1, 1), ps = 7, lwd = 0.5)
plot(c(0, length(bp.go.gene)), c(0, length(bp.go.gene)), type = "n", axes = FALSE, xlab = "", ylab = "", asp = 1)
image(1:length(bp.go.gene), 1:length(bp.go.gene), old.order.cor, col = colorRampPalette(c(brewer.pal(9, "Blues")[9], "white", brewer.pal(9, "Reds")[9]))(101), useRaster = F, add = TRUE)
...
text(1:length(bp.go.gene) + 1.1, rep(0, length(bp.go.gene)), gene.info[rownames(young.order.cor$cor), 2], font = 3, srt = 45, pos = 2, xpd = TRUE, col = ifelse(gene.info[rownames(young.order.cor$cor), 2] %in% c("kdn", "ScsbetaA"), "red", "black"))
text(0, length(bp.go.gene) + 2, "Old", pos = 4, cex = 7/par("ps")/par("cex"), xpd = TRUE)
...
par(las = 1, tcl = -0.2, mar = c(0.5, 0.5, 0.5, 0.5), ps = 7, lwd = 0.5)
plot.igraph(young.graph, layout = old.layout, vertex.color = brewer.pal(12, "Set3")[5], vertex.size = 20, vertex.label.color = "black", vertex.frame.color = brewer.pal(12, "Set3")[5], edge.width = 0.5, vertex.label.font = 3, vertex.label.cex = 2.2/par("ps")/par("cex"), vertex.label.family = args[5])
text(mean(par('usr')[1:2]), min(par('usr')[3:4]), "Young", pos = 2, adj = 2, cex = 7/par("ps")/par("cex"))
...
par(las = 1, tcl = -0.2, mar = c(0.5, 0.5, 0.5, 0.5), ps = 7, lwd = 0.5)
plot.igraph(old.graph, layout = old.layout, vertex.color = brewer.pal(12, "Set3")[5], vertex.size = 20, vertex.label.color = "black", vertex.frame.color = brewer.pal(12, "Set3")[5], edge.width = 0.5, vertex.label.font = 3, vertex.label.cex = 2.2/par("ps")/par("cex"), vertex.label.family = args[5])

text(mean(par('usr')[1:2]), min(par('usr')[3:4]), "Old", pos = 2, adj = 2, cex = 7/par("ps")/par("cex"))
