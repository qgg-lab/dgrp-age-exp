# =====================================================
# = make a table for the Aging GSEA analysis =
# =====================================================

args <- commandArgs(TRUE) # args <- c("../figureData/female.age.effect.gsea.fdr.RData", "../figureData/male.age.effect.gsea.fdr.RData", "../figureData/female.age.effect.kegg.fdr.RData", "../figureData/male.age.effect.kegg.fdr.RData", "../figureData/kegg.id")
library("openxlsx")
library("GO.db")
...
load(args[3])
female.kegg <- unname(cbind("KEGG", as.character(kegg.pos.fdr[, 1]), kegg.id[as.character(kegg.pos.fdr[, 1]), 2], kegg.pos.fdr[, 2:3],
                          as.character(kegg.neg.fdr[, 1]), kegg.id[as.character(kegg.neg.fdr[, 1]), 2], kegg.neg.fdr[, 2:3]))

load(args[4])
male.kegg <- unname(cbind("KEGG", as.character(kegg.pos.fdr[, 1]), kegg.id[as.character(kegg.pos.fdr[, 1]), 2], kegg.pos.fdr[, 2:3],
                                                    as.character(kegg.neg.fdr[, 1]), kegg.id[as.character(kegg.neg.fdr[, 1]), 2], kegg.neg.fdr[, 2:3]))
