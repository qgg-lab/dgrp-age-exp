...
blup.25c.soft.threshold = pickSoftThreshold(blup.25c, dataIsExpr = TRUE, powerVector = power.scan, corFnc = cor, corOptions = list(use = 'pairwise', method = "pearson"), networkType = "signed", RsquaredCut = 0.90)

blup.25c.adj <- adjacency(blup.25c, power = blup.25c.soft.threshold$powerEstimate, corOptions = list(use = 'pairwise', method = "pearson"), type = "signed")
...
blup.18c.adj <- adjacency(blup.18c, power = blup.25c.soft.threshold$powerEstimate, corOptions = list(use = 'pairwise', method = "pearson"), type = "signed")
...
names(blup.18c.tree) <- colnames(blup.18c)

save(blup.25c.soft.threshold, blup.25c.cor, blup.25c.tom, blup.25c.clust, blup.25c.tree, blup.18c.cor, blup.18c.tom, blup.18c.clust, blup.18c.tree, file = args[2])
