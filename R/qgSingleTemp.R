...
single.temp <- foreach (i = 1:nrow(gene.exp.adj), .combine = rbind) %dopar% {
  ...
  lmer.fit.18c <- lmer(y.18c ~ (1|line.18c)) # rand(lmer.fit.18c)$Pr[2] is p value for random effect
  # as.data.frame(VarCorr(lmer.fit.18c))[, 4] is variance components
  lmer.fit.25c <- lmer(y.25c ~ (1|line.25c)) 
  res[1:3] <- c(as.data.frame(VarCorr(lmer.fit.18c))[, 4], rand(lmer.fit.18c)$Pr[2])
  res[4:6] <- c(as.data.frame(VarCorr(lmer.fit.25c))[, 4], rand(lmer.fit.25c)$Pr[2])
  ...
  res[7:9] <- c(as.data.frame(VarCorr(lmer.fit.18c.wolba))[, 4], rand(lmer.fit.18c.wolba)$Pr[2])
  res[10:12] <- c(as.data.frame(VarCorr(lmer.fit.25c.wolba))[, 4], rand(lmer.fit.25c.wolba)$Pr[2])
