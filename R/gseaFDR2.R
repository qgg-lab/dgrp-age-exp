...
for (i in 1:length(bp.es.pos.sorted)) {
  ...
  bp.pos.fdr[i, 3] <- (sum(bp.mean.es.pos.perm >= bp.es.pos.sorted[i]) + 1)/(ncol(bp.es.perm)/2 + 1)/i
  
}

for (i in 1:length(bp.es.neg.sorted)) {
  
  bp.neg.fdr[i, 3] <- (sum(bp.mean.es.neg.perm >= bp.es.neg.sorted[i]) + 1)/(ncol(bp.es.perm)/2 + 1)/i
...
for (i in 1:length(mf.es.pos.sorted)) {
  ...
  mf.pos.fdr[i, 3] <- (sum(mf.mean.es.pos.perm >= mf.es.pos.sorted[i]) + 1)/(ncol(mf.es.perm)/2 + 1)/i
  
}

for (i in 1:length(mf.es.neg.sorted)) {
  
  mf.neg.fdr[i, 3] <- (sum(mf.mean.es.neg.perm >= mf.es.neg.sorted[i]) + 1)/(ncol(mf.es.perm)/2 + 1)/i
...
for (i in 1:length(cc.es.pos.sorted)) {
  ...
  cc.pos.fdr[i, 3] <- (sum(cc.mean.es.pos.perm >= cc.es.pos.sorted[i]) + 1)/(ncol(cc.es.perm)/2 + 1)/i
  
}

for (i in 1:length(cc.es.neg.sorted)) {
  
  cc.neg.fdr[i, 3] <- (sum(cc.mean.es.neg.perm >= cc.es.neg.sorted[i]) + 1)/(ncol(cc.es.perm)/2 + 1)/i
