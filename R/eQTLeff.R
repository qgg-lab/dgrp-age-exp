...
args <- commandArgs(TRUE) # args <- c("eqtl/female.18c.fdr05.model.select.RData", "eqtl/female.18c.eqtl.fdr05.out", "eqtl/female.18c.genvar.id", "eqtl/female.25c.fdr05.model.select.RData", "eqtl/female.25c.eqtl.fdr05.out", "eqtl/female.25c.genvar.id", "eqtl/eqtl.fdr05.snp.geno.tped", "eqtl/eqtl.fdr05.snp.geno.tfam", "eqtl/female.18c.pheno", "eqtl/female.25c.pheno")
...
for (i in 1:nrow(eqtl.pair)) {
  ...
  if (sum(col.names.18c == eqtl.pair[i, 1]) > 0) {
    ...
    eqtl.pair[i, 6] <- var(pheno.18c[, eqtl.pair[i, 1]], na.rm = TRUE)
  }
  if (sum(col.names.25c == eqtl.pair[i, 1]) > 0) {
    ...
    eqtl.pair[i, 9] <- var(pheno.25c[, eqtl.pair[i, 1]], na.rm = TRUE)
  }
}
